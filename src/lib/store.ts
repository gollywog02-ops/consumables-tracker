...
function load(): DB {
  ...
  try { return JSON.parse(raw) as DB; }
  catch { return { locations: [], items: [], transactions: [], version: 1 }; }
}
...
export function addLocation(name: string) {
  const db = load(); db.locations.push({ id: uid(), name: name.trim() }); save(db); return db.locations.at(-1)!;
}
export function addItem(data: Omit<Item, "id">) {
  const db = load(); const item = { ...data, id: uid() }; db.items.push(item); save(db); return item;
}
export function addTransaction(data: Omit<Transaction, "id" | "createdAt">) {
  const db = load(); const txn: Transaction = { ...data, id: uid(), createdAt: new Date().toISOString() };
  db.transactions.push(txn); save(db); return txn;
}

export function balancesByItemLocation() {
  const db = load(); const map = new Map<string, number>();
  for (const t of db.transactions) {
    ...
    map.set(key, (map.get(key) ?? 0) + sign * t.qty);
  }
  return map;
}

export function lowStock() {
  const db = load(); const bal = balancesByItemLocation(); const out: { item: Item; location: Location; onHand: number }[] = [];
  for (const item of db.items) {
    for (const loc of db.locations) {
      const key = `${item.id}::${loc.id}`; const onHand = bal.get(key) ?? 0;
      if (onHand <= item.reorderPoint) out.push({ item, location: loc, onHand });
    }
  }
  return out;
}
...
export function exportJSON() {
  const db = load(); const blob = new Blob([JSON.stringify(db, null, 2)], { type: "application/json" });
  const url = URL.createObjectURL(blob); const a = document.createElement("a");
  a.href = url; a.download = "consumables_tracker_backup.json"; a.click(); URL.revokeObjectURL(url);
}
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      try {
        ...
        if (!db || !Array.isArray(db.items) || !Array.isArray(db.locations) || !Array.isArray(db.transactions)) throw new Error("Invalid file");
        localStorage.setItem(KEY, JSON.stringify(db)); resolve();
      } catch (e) { reject(e); }
    };
