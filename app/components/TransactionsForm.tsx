...
export default function TransactionsForm() {
  ...
  return (<div style={{ padding: 12, border: "1px solid #eee", borderRadius: 12 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
      <select value={itemId} onChange={e=>setItemId(e.target.value)}>{db.items.map(i => <option key={i.id} value={i.id}>{i.sku} — {i.name}</option>)}</select>
      <select value={locationId} onChange={e=>setLocationId(e.target.value)}>{db.locations.map(l => <option key={l.id} value={l.id}>{l.name}</option>)}</select>
