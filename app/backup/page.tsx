  ...
  return (<main style={{ display: "grid", gap: 16 }}>
    ...
    <div>
      <input type="file" accept="application/json" onChange={async (e)=>{
        ...
        try { await importJSON(f); setMsg("Import complete ✅. Reload the page."); }
        catch (e:any) { setMsg("Import failed: " + e.message); }
      }} />
    </div>
    <details><summary>Preview current data</summary><pre style={{ whiteSpace: "pre-wrap" }}>{JSON.stringify(getDB(), null, 2)}</pre></details>
    {msg && <p>{msg}</p>}
  </main>);
}
